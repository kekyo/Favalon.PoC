...
namespace Favalet.Parsers.Runners
{
    internal sealed class NumericalSignedRunner : ParseRunner
    {
        ...
        {
            var sign = context.GetAndClearPreSignToken().Symbol.ToString();

            switch (token)
            {
                // "-123"
                case NumericToken numeric:
                    context.Combine(context.Features.CreateNumeric(sign + numeric.Value));
                    return ApplyingRunner.Instance;

                // "-abc"
                case IdentityToken identity:
                    context.Combine(context.Features.CreateIdentity(sign));

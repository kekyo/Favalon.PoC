...
namespace Favalet.Parsers
{
    ...
    internal static class ParserUtilities
    {
...
#endif
        public static IExpression CombineTerms(IExpression? left, IExpression? right)
        {
            if (left != null)
            {
                if (right != null)
        ...
        public static IExpression? HideTerm(IExpression? term) =>
            ...
            if (runnerContext.CurrentContext.LookupBoundTerms(identity.Identity) is BoundTermInformation[] terms)
            {
                ...
                if (terms[0].Precedence is BoundTermPrecedences termPrecedence)
                {
                    ...
                    if (runnerContext.CurrentPrecedence is BoundTermPrecedences precedence)
                    {
                        ...
                        {
                            // (Transpose SavedTerm)
                            if (runnerContext.CurrentTerm is ApplyTerm(IExpression left, IExpression right))
                            {
